...
jsc*/
   ...
   * @private
   */
  define("tinymce.plugins.template.Plugin", [
    ...
    PluginManager.add("template", function (editor) {
      ...
      function showDialog(templateList) {
        ...
        win = editor.windowManager.open({
          ...
          minWidth: Math.min(
            DOMUtils.DOM.getViewPort().w,
            editor.getParam("template_popup_width", 600),
          ),
          minHeight: Math.min(
            DOMUtils.DOM.getViewPort().h,
      ...
      function getDateTime(fmt, date) {
        ...
        fmt = fmt.replace(
          "%B",
          "" + editor.translate(monthsLong[date.getMonth()]),
        );
        fmt = fmt.replace(
          "%b",
        ...
        fmt = fmt.replace(
          "%a",
          "" + editor.translate(daysShort[date.getDay()]),
        );
        fmt = fmt.replace("%%", "%");
      ...
      function replaceVals(e) {
        var dom = editor.dom,
          vl = editor.getParam("template_replace_values");
      ...
      function insertTemplate(ui, html) {
        ...
        Tools.each(dom.select("*", el), function (n) {
          ...
          if (
            ...
            n.innerHTML = getDateTime(
              editor.getParam(
                "template_cdate_format",
                editor.getLang("template.cdate_format"),
              ),
            );
          }
          ...
          if (
            ...
            n.innerHTML = getDateTime(
              editor.getParam(
                ...
                editor.getLang("template.mdate_format"),
              ),
            );
          }
