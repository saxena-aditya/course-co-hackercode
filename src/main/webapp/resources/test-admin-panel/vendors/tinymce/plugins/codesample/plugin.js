...
jsc*/
  ...
  define("tinymce.plugins.codesample.core.Prism", [], function () {
    var window = {};
    // ------------------ Start wrap
   ...
   * @private
   */
  define("tinymce.plugins.codesample.util.Utils", [], function () {
   ...
   * @private
   */
  define("tinymce.plugins.codesample.ui.Dialog", [
    ...
    function getCurrentLanguage(editor) {
      var matches,
        node = getSelectedCodeSample(editor);
    ...
    return {
      open: function (editor) {
        editor.windowManager.open({
          ...
          minWidth: Math.min(
            DOM.getViewPort().w,
            editor.getParam("codesample_dialog_width", 800),
          ),
          minHeight: Math.min(
            DOM.getViewPort().h,
            editor.getParam("codesample_dialog_height", 650),
          ),
          layout: "flex",
          direction: "column",
          align: "stretch",
   ...
   * @private
   */
  define("tinymce.plugins.codesample.Plugin", [
    "tinymce.core.Env",
    ...
    PluginManager.add("codesample", function (editor, pluginUrl) {
      ...
      function loadCss() {
        var linkElm,
          contentCss = editor.settings.codesample_content_css;
