...
jsc*/
  ...
  define("tinymce.plugins.textpattern.core.Patterns", [], function () {
    ...
    var findPattern = function (patterns, text) {
      for (var i = 0; i < patterns.length; i++) {
        ...
        if (
          patterns[i].end &&
          text.lastIndexOf(patterns[i].end) !==
            text.length - patterns[i].end.length
        ) {
          continue;
        }
    ...
    var isMatchingPattern = function (pattern, text, offset, delta) {
      var textEnd = text.substr(
        offset - pattern.end.length - delta,
        pattern.end.length,
      );
  ...
  define("tinymce.plugins.textpattern.core.Formatter", [
    ...
    var splitContainer = function (
      ...
      container.deleteData(
        container.data.length - pattern.end.length,
        pattern.end.length,
      );
    ...
    var applyInlineFormat = function (editor, patterns, space) {
      ...
      startOffset = text.lastIndexOf(
        pattern.start,
        startOffset - pattern.end.length - 1,
      );
  ...
  define("tinymce.plugins.textpattern.core.KeyHandler", [
  ...
  define("tinymce.plugins.textpattern.core.Settings", [], function () {
    var defaultPatterns = [
   ...
   * @private
   */
  define("tinymce.plugins.textpattern.Plugin", [
