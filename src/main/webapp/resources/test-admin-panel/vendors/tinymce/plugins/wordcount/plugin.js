...
jsc*/
  ...
  define("tinymce.plugins.wordcount.text.UnicodeData", [], function () {
    var regExps = {
      aletter:
  ...
  define("tinymce.plugins.wordcount.alien.Arr", [], function () {
  ...
  define("tinymce.plugins.wordcount.text.WordBoundary", [
    ...
    var isWordBoundary = function (map, index) {
      ...
      if (
        type === ci.ALETTER &&
        (nextType === ci.MIDLETTER ||
          nextType === ci.MIDNUMLET ||
          nextType === ci.AT) &&
      ...
      // adjacent to letters.
      if (
        (type === ci.NUMERIC || type === ci.ALETTER) &&
      ...
      // "3,456.789".
      if (
        (type === ci.MIDNUM || type === ci.MIDNUMLET) &&
        nextType === ci.NUMERIC &&
        prevType === ci.NUMERIC
      ) {
        return false;
