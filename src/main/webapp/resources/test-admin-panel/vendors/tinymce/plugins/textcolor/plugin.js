...
jsc*/
   ...
   * @private
   */
  define("tinymce.plugins.textcolor.Plugin", [
    ...
    PluginManager.add("textcolor", function (editor) {
      ...
      rows = {
        forecolor:
          editor.settings.forecolor_rows || editor.settings.textcolor_rows || 5,
        backcolor:
          editor.settings.backcolor_rows || editor.settings.textcolor_rows || 5,
      };
      cols = {
        forecolor:
          editor.settings.forecolor_cols || editor.settings.textcolor_cols || 8,
        backcolor:
          editor.settings.backcolor_cols || editor.settings.textcolor_cols || 8,
      };
      ...
      function getCurrentColor(format) {
        ...
        editor.dom.getParents(editor.selection.getStart(), function (elm) {
          ...
          if (
            (value =
              elm.style[format == "forecolor" ? "color" : "background-color"])
          ) {
            color = value;
          }
        });
      ...
      function onPanelClick(e) {
        ...
        if (DOMUtils.DOM.getParent(e.target, ".mce-custom-color-btn")) {
          ...
          editor.settings.color_picker_callback.call(
            editor,
            function (value) {
              var tableElm = buttonCtrl.panel
                .getEl()
                .getElementsByTagName("table")[0];
              var customColorCells, div, i;

              customColorCells = Tools.map(
