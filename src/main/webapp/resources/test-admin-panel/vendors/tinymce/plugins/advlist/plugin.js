...
jsc*/
   ...
   * @private
   */
  define("tinymce.plugins.advlist.Plugin", [
    ...
    PluginManager.add("advlist", function (editor) {
      ...
      function isListNode(node) {
        return (
          node && /^(OL|UL|DL)$/.test(node.nodeName) && isChildOfBody(node)
        );
      }
      ...
      function applyListFormat(listName, styleValue) {
        editor.undoManager.transact(function () {
          ...
          if (list) {
            Tools.each(
              dom.select("ol,ul", list).concat([list]),
              function (list) {
