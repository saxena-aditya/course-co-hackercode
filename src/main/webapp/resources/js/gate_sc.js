...
function fetch_q3(k) {
  if (um == 1) {
    ...
    if (qlog[current] == 1) qlog[current] = 0;
    ...
    if ($("#form-0").find("#dispInp" + current).length) {
      if ($('input:text[name="' + current + 'anss"]').val().length) {
        $("#form-0")
          .find("#dispInp" + k)
          .find("#indiv")
          .html($('input:text[name="' + current + 'anss"]').val());
    ...
    if (qlog[k] == 0) $("#" + k + "d").attr("class", "not_answered");
  }
...
function save_next() {
  if (
    ...
    if (cur_ans != "null") {
      ...
      if (qlog[current] == 4) $("#" + current + "d").attr("class", "answered");
