...
function startTest() {
  ...
  if (
    $.cookie(ssid + tid + "test") &&
    $.cookie(ssid + tid + "test") == "progress"
  ) {
...
function fetch_all(testid) {
  $.ajax({
    url: "prepareTest.php?getTest=" + testid + "&take=" + take,
  ...
  $.ajax({
    ...
    error: function () {
      document.getElementById("container").innerHTML =
        "<h1 style='text-align:center;margin:35px;' >Wait There is Network Congestion.</h1><p id='res_box' ><span style='color:red;font-weight:bold;' >Please do not Close this Window! Keep Try Re-Submitting.</span> <br/><br/><input style='padding:15px;' type='button' value='Submit Test' onclick='submitTest();' ></p>";
    },
  });
  /*
...
function testOver() {
  ...
  document.getElementById("container").innerHTML =
    "<h1 style='text-align:center;margin:35px;' >Your Test is Over, Submit When you are asked to Submit.</h1><p id='res_box' ><input style='padding:15px;' type='button' value='Submit Test' onclick='submitTest();' ></p>";
  ...
  $.ajax({
    url: "prepareTest.php?over=" + tid + "&ssid=" + ssid,
    success: function (result) {
      //alert(result);
    },
  });
...
function conf_fin() {
  $("#msg_area").html(
    "<div style='border-radius:10px;opacity:0.8;padding:10px;width:600px;position:fixed;top:40%;left:25%;background-color:#000000;color:#ffffff;text-align:center;'><h2>Confirm Finished?</h2><p style='padding:15px;'><input type='button' value='Finish' onclick='testOver();' /> <input type='button' value='Cancel' onclick='$(\"#msg_area\").hide();' /></p></div>",
  );
...
function review() {
  ...
  if (
    ...
    qlog[current] == 2 ||
    qlog[current] == 4 ||
    (qlog[current] == 3 && cur_ans != "null") ||
    ($("#form-0").find("#dispInp" + current).length &&
      $("#form-0")
        .find("#dispInp" + current)
        .find("#indiv")
        .html() != "|")
    ...
    if (current == total) fetch_nRep(1);
    else fetch_nRep(current + 1);
  } else {
    qlog[current] = 3;
    $("#" + current + "d").attr("class", "review");

    if (current == total) fetch_q3(1);
    else fetch_q3(current + 1);
  }
}
...
function takeFeedback(k) {
  ...
  $("#" + e).fadeOut("slow");
  if (k == "H") $("#" + e).html("Marked as Hard.");
  else if (k == "M") $("#" + e).html("Marked as Medium.");
  else $("#" + e).html("Marked as Easy.");
  $("#" + e).fadeIn("fast");
}
